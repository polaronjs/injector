...
export type InjectableConfig = {
  /**
   * Overwrite existing tokens the event the Injector attempts to register a token that already exists
   */
  force?: boolean;

  /**
   * For the given token, instead of registering it to an instance of itself, use the provided value
   */
  useValue?: any;

  /**
   * For the given token, instead of registering it to an instance of itself, use a new instance of the provided class
   */
...
export class Injector {
   ...
   * @param config
   */
  static register(token: Injectable<any> | any, config?: InjectableConfig) {
    if (!this.instance) {
    ...
    if (config && config.useValue) {
      ...
      if (this.instance.injectables.has(token) && !config.force) {
        throw new Error(
          `Cannot register token \`${token.name}\` with Injector: token exists. Try using the 'force' configuration option.`
        );
      }
